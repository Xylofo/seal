...
\section{Algoritmer för att generera sekvensen}
    ...
    \subsubsection{Mainloop}
        Slutligen sätter vi ihop allt till en mainloop som fyller up
        ...
        man redan innan fylla upp dessa möjligheter när man behandlar en delare
        till $d$ till $n$ och då minska tidskomplexiteten från $\O(n\sqrt n)$
        ...
        n\log(n)\log(\log(n))  = \O(n^{\beta+1}) \approx \O(n^1.585)$. Dock kommer det i de flesta
        fall vara så att $kMax_n << 2n^\beta$. Detta gör det möjligt att bevisa
        den starkare övre gränsen på $\O(n^\alpha)$ där $\alpha\approx
        1.230175$\,\,\cite{algorithm_lune}. Denna gräns på $\alpha$ är dock inte
        nödvändigtvis den minsta möjliga.
...
bättre minneskomplexitet. Detta gör den mer praktiskt användbar då det som
begränsar den ovan beskrivna algoritmen är dess minneskomplexitet $\O(n
\log(\log(n)))$ då vi sparar $n$ tal med storlek $\O(log n)$ (gräns på $\C{n}$).
Att spara ett tal med storlek $k$ tar $\O(log k)$ minne. Idén med Fullers
algoritm är att endast spara vissa delar av $C[\cdot]$ vid varje tidpunkt i
minnet. Vilka delar som behöver sparas är ett relativt svårt problem vilket är
därför vi inte förklarar algoritmen här. Denna algoritm är den som är använd för
att kunna undersöka egenskaper av $\C{n}$, exempelvis dess invers, hur tajta
gränserna är och hitta motexempel till hypoteser.
...
Optimerad algoritm & $\O(n^{1.230175})$ & 0.6584 \\ \hline
