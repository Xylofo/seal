...
\begin{abstract}
    ...
    The first part of the report discusses and presents proofs and illustrations for the bounds for functions
    of integer complexity.
    The report also presents investigations on specific groups of integers, such as prime numbers and
    powers of 3, presenting narrower bounds, explicit formulas.
...
\section{Inledning}
 ...
 \begin{tabular}{| l | l | l | }
    \hline
    n & $\C{n}$ & exempelminumumrepresentation  \\ \hline
    1 & 1 & 1  \\ \hline
    2 & 2 & 1+1  \\ \hline
    3 & 3 & 1+1+1  \\ \hline
    4 & 4 & 1+1+1+1  \\ \hline
...
\end{center}
	...
	Frågor om storleken på heltalens komplexitet har ofta enkla formuleringar som är enkla att 	förstå utan en matematisk utbildning. Svaren på frågorna verkar betydligt svårare och kan 	ge insikt i ett antal andra frågor.	
	
	\subsubsection{Ett exempel}
	Tidskomplexiteten för att räkna ut komplexiteten av ett 
	givet heltal är kopplat till problemet är $P \stackrel{?}{=} NP$
	
	$P$ och $NP$ kan illustreras med följande exempel. 
	Inför ett alfabet,  mängd, A,
	en mängd A* 	av alla finita ord som kan skrivas med A,
	ett språk S som är en delmängd av A*,
	dä är  S tillhörande P omm det existerar en algoritm T och ett polynom P så att T ger T(x)=1 	om x är ett element ur S och T(x)=0 om x ej är ett element ur S, samt att T ger T(x) på en tid 	$\le$ P(|x|), där |x| betecknar längden av ordet x.
	S tillhör NP omm det existerar en algoritm T och ett polynom P så att det för alla x existerar ett 	y så att |y|$\le$P(|x|) och T ger T(x,y)=1 om x år ett element ur S på en tid $\le$ P(|x|) och T
	ger T(x,y)=0 omm x ej tillhör S.
	
		
	En koppling är att $N = NP$  implicerar att det existerar en algoritm T för att räkna ut 		komplexiteten av heltal n som har tidskomplexitet $\le$  $C (\log n)^k$. 
	nogot om talrepresentation. 
	En annan koppling är att $N = NP$  implicerar att det inte existerar en talföljd av naturliga tal 
	$a$ så att $\stackrel{\lim}{x\to\infty} \frac{\C{a}}{\log{a}} \le \frac{3}{\log{3}}$
...
\section{Analys}
    ...
    lika, men formen är något förskjuten. Faktum är att dalen i det röda intervallet sammanfaller med högsta punkten
    i det röda. Vad som orsakar att formen blir så här fann vi inget svar på. Formen blir inte mer och mer förskjuten till höger
    ...
    samband existerar för specifika typer av tal. 
        ...
        \begin{hypot}
            ...
            } \C{4\cdot3^k} = 3k+4$. Detta följer av funktionenerna $E$ och $B$ som
...
\section{Algoritmer för att generera sekvensen}
    ...
    Dessutom kan man göra algoritmen iterativ (dvs. att den inte är
    ...
    $$\min_{1 \leq k \leq \floor{n/2}} \C{k} + \C{n-k} = \min_{1 \leq k \leq kMax} \C{k} + \C{n-k}$$
    ...
    Denna algoritm beräknar, med hjälp av $E$, tidigare nämna $kMax_n$. 
    ...
    \subsubsection{Mainloop}
        ...
        Totalt kommer alltså tidskomplexiteten vara $\O(n\cdot kMax_n +
        n\log(n)\log(\log(n))) = \O(n^{\beta+1}) \approx \O(n^{1.585})$. Dock
        kommer det i de flesta fall vara så att $kMax_n \ll 2n^\beta$. Detta gör
    ...
    \subsection{Fler algoritmer och diskussion}
        ...
        användbarhet. Den första är den enklaste möjliga algoritmen för
        ...
        optimerade algoritm som finns för tillfället, är Fuller's algoritm, som
        ...
        här. Denna algoritm är den som är använd för att kunna undersöka
        egenskaper av $\C{n}$, exempelvis dess invers, hur tajta gränserna är
        ...
        som tidigare nämnt några oväntade användningsområden (även om de inte är
        särskilt stora). Att studera algoritmer för att lösa detta problem visar
        ...
        relaterade till heltalskomplexiteten, bland annat delbarhet och primtals
        såll. Att finna nya strategier inom detta problem kan alltså även ge
